...
| 패턴 | 설명 | 난이도 범위 |
|------|------|------------|
| **트릴** | 두 트랙에 배치된 노트가 번갈아 나오는 교차 연타 | 0-20 |
| **계단** | 계단을 옆에서 본 것처럼 노트가 배치된 패턴 | 0-20 |
| **동치** | 여러 개의 시퀀스를 동시에 치는 패턴 | 0-20 |
| **데님** | 135-24 / 1357-246 식으로 거미줄처럼 짜인 배치 | 0-20 |
| **따닥이** | 짧은 연타와 잡노트가 섞인 배치, 고속 처리 | 0-20 |
| **롱잡** | 롱노트를 처리하는 중에 다른 노트를 처리 | 0-20 |
| **폭타** | 순간적/지속적으로 많은 노트 처리 필요 | 0-20 |
| **즈레** | 정박에서 약간 어긋나는 엇박 패턴 | 0-20 |
...
트릴 레이더 점수 = (14.25 + 16.20 + 11.76) / 3 = **14.07**
